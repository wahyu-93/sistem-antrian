...
class ThermalPrinterService
{
    ...
    {
        $data = $qrParams['data'] ?? '';
        $size = max(4, min(16, $qrParams['size'] ?? 10)); // Ukuran aman
        $eccLevel = max(48, min(51, $qrParams['eccLevel'] ?? 49)); // M
        $align = $qrParams['align'] ?? 'center';

        $qrCode = "\n" . $this->align($align); // Feed atas + align
        ...
        // Store data
        $dataLen = strlen($data) + 3;
        $qrCode .= "\x1D\x28\x6B" .
            chr($dataLen % 256) . chr($dataLen >> 8) .
            "\x31\x50\x30" . $data;
        // Print QR Code
